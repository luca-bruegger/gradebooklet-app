...
export class SubjectViewComponent implements OnInit {
  ...
  async closeModal() {
    JSON.stringify(this.editModule) === JSON.stringify(this.editModuleBackup) ?
      ...
      await this.popupController.notSavedPopup();
  }
  ...
  async prepareModuleSave() {
    if (!!this.editModule.name) {
      if (this.examComponent.areExamsValid()) {
        ...
        await this.popupController.genericPopup(this.translateService.instant('popup.warning'),
  ...
  async promptUser() {
    if (this.editModule.exams.length > 0) {
      await this.popupController.gradeErrorPopup(this.translateService.instant('popup.warning-grades-deletion'));
    }
  }
